...
const traceArrayExpression = (node: TSESTree.Node, context: TSESLint.SourceCode, verify: (node: TSESTree.Node) => boolean): ITraceValueReturn => {
    ...
    const results = node.elements.map(arrValue => {
        ...
        else if (localArrValue.type === AST_NODE_TYPES.SpreadElement) return innerTraceValue(localArrValue.argument, context, verify);
        else return innerTraceValue(localArrValue, context, verify);
    });
