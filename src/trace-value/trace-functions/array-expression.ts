...
const traceArrayExpression = (node: TSESTree.Node, context: TSESLint.SourceCode, verify: (node: TSESTree.Node) => boolean, nodeTrace: ITraceNode): ITraceValueReturn => {
    ...
    const results = node.elements.map(arrValue => {
        ...
        if (!localArrValue) return { result: { isVerified: true, determiningNode: node }, nodeComponentTrace: nodeTrace };
    ...
    if (unverifiedNode) return { result: { isVerified: false, determiningNode: unverifiedNode.result.determiningNode }, nodeComponentTrace: unverifiedNode.nodeComponentTrace };
