...
const traceArrowFunctionExpression = (node: TSESTree.Node, context: IRuleContext, verify: (node: TSESTree.Node) => boolean): ITraceValueReturn => {
  ...
  if (node.body.type === AST_NODE_TYPES.BlockStatement) {
    ...
    const result = node.params.length > 0 ? innerTraceValue(node.body, context, verify, { functionParams: node.params }) : innerTraceValue(node.body, context, verify);
